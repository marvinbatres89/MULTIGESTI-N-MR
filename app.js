(() => {
  ...
  async function registerFreshWorker(){if(!('serviceWorker' in navigator)||!location.protocol.startsWith('http'))return;try{const regs=await navigator.serviceWorker.getRegistrations();for(const reg of regs){try{await reg.update()}catch(_){}}await navigator.serviceWorker.register('./sw.js?v=132',{updateViaCache:'none'});}catch(err){console.warn('SW',err)}}
  // ---- NUBE / AUTENTICACION / ENLACES V1.3.2 ----
  function cloudMessage(message,type='info',target='authMessage'){
    const box=$(target);if(!box)return toast(message);
    box.textContent=message;box.className=`auth-message ${type}`;box.hidden=false;
  }
  function clearCloudMessage(target='authMessage'){const box=$(target);if(!box)return;box.hidden=true;box.textContent='';box.className='auth-message';}
  function openRecoveryDialog(){
    try{if($('authDialog')?.open)$('authDialog').close();}catch(_){}
    try{if(!$('resetPasswordDialog')?.open)$('resetPasswordDialog')?.showModal();}catch(_){}
    cloudMessage('Enlace de recuperación validado. Escriba y confirme su nueva contraseña.','success','resetPasswordMessage');
  }
  $('openAuthBtn')?.addEventListener('click',()=>{clearCloudMessage();$('authDialog').showModal();});
  $('closeAuthDialog')?.addEventListener('click',()=>$('authDialog').close());
  $('authForm')?.addEventListener('submit',async e=>{e.preventDefault();clearCloudMessage();cloudMessage('Conectando con la nube…','info');try{await cloud().signIn(normalize($('authEmail').value),$('authPassword').value);cloudMessage('Nube conectada correctamente.','success');activeBusinessId=null;await refresh();setTimeout(()=>{$('authDialog')?.close();toast('Nube conectada');},650);}catch(err){cloudMessage(err.message||'No fue posible iniciar sesión','error');}});
  $('signUpBtn')?.addEventListener('click',async()=>{clearCloudMessage();const email=normalize($('authEmail').value),password=$('authPassword').value,name=normalize($('authName').value);if(!email||password.length<6)return cloudMessage('Ingrese correo y una contraseña de al menos 6 caracteres.','error');cloudMessage('Creando acceso administrador…','info');try{const data=await cloud().signUp(email,password,name);if(data.session){cloudMessage('Administrador creado y conectado.','success');await refresh();setTimeout(()=>{$('authDialog')?.close();toast('Administrador creado y conectado');},650);}else cloudMessage('Cuenta creada. Revise el correo de confirmación y luego inicie sesión.','success');}catch(err){cloudMessage(err.message||'No se pudo crear el acceso','error');}});
  $('forgotPasswordBtn')?.addEventListener('click',async()=>{clearCloudMessage();const email=normalize($('authEmail').value);if(!email){cloudMessage('Escriba primero el correo del administrador.','error');$('authEmail')?.focus();return;}cloudMessage('Enviando correo de recuperación…','info');try{await cloud().resetPasswordForEmail(email);cloudMessage('Correo de recuperación enviado. Revise su bandeja de entrada y abra el enlace recibido.','success');}catch(err){cloudMessage(err.message||'No se pudo enviar el correo de recuperación','error');}});
  $('closeResetPasswordDialog')?.addEventListener('click',()=>$('resetPasswordDialog')?.close());
  $('resetPasswordForm')?.addEventListener('submit',async e=>{e.preventDefault();clearCloudMessage('resetPasswordMessage');const password=$('newPassword')?.value||'',confirmPassword=$('confirmNewPassword')?.value||'';if(password.length<6)return cloudMessage('La contraseña debe tener al menos 6 caracteres.','error','resetPasswordMessage');if(password!==confirmPassword)return cloudMessage('Las contraseñas no coinciden.','error','resetPasswordMessage');cloudMessage('Guardando la nueva contraseña…','info','resetPasswordMessage');try{await cloud().updatePassword(password);$('resetPasswordForm')?.reset();cloudMessage('Contraseña actualizada correctamente. Ya puede iniciar sesión.','success','resetPasswordMessage');setTimeout(()=>{$('resetPasswordDialog')?.close();toast('Contraseña actualizada correctamente');},900);}catch(err){cloudMessage(err.message||'No se pudo actualizar la contraseña','error','resetPasswordMessage');}});

  ...
  */
  ...
  $('cloudDiagnosticBtn')
    ?.addEventListener(
      'click',
      async () => {
        const panel = $('cloudDiagnosticPanel');
        if (!panel) return;
        panel.hidden = false;
        panel.className = 'auth-message cloud-diagnostic info';
        panel.textContent = 'Consultando Supabase…';
        try {
          const d = await cloud().getCloudDiagnostic();
          const lines = [
            `Sesión activa: ${d.sessionActive ? 'SÍ' : 'NO'}`,
            `Usuario: ${d.userIdShort}`,
            `Correo: ${d.emailMasked}`,
            `Negocios devueltos: ${d.businesses.length}`
          ];
          d.businesses.forEach((b, i) => {
            lines.push(`${i + 1}. ${b.name} · ${b.access} · ID ${b.idShort}`);
          });
          ...
          panel.className = 'auth-message cloud-diagnostic success';
        } catch (err) {
          panel.textContent = `No se pudo completar el diagnóstico: ${err?.message || err}`;
          panel.className = 'auth-message cloud-diagnostic error';
        }
      }
    );

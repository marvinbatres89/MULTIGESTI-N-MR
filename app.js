(() => {
  ...
  */
    ...
    if (cloudActive()) {
      try {
        ...
        if (
          ...
          remote.warnings.length
        ) {
          console.warn(
            '[MULTIGESTIÓN MR] Sincronización parcial:',
            remote.warnings
          );
          toast(
            'Nube: negocios cargados; algunos módulos secundarios no pudieron sincronizar.'
          );
        }
        ...
        */
        ...
        /*
          V1.4.3:
          Si existe una sesión autenticada, Supabase es
          la única fuente de verdad. Nunca reutilizamos
          negocios/movimientos antiguos de IndexedDB
          cuando falla la sincronización, porque eso puede
          mostrar proyectos eliminados o de pruebas pasadas.
        */
        toast(
          `Nube: ${
            err.message ||
      ...
      /*
        V1.4.5 - NO MEZCLAR NUBE CON DATOS LOCALES
        ------------------------------------------------
        Si el módulo Supabase está configurado pero no existe una
        sesión autenticada, NO cargamos businesses/movements desde
        IndexedDB. Esa era la ruta que podía mostrar negocios antiguos
        (por ejemplo, un proyecto local de pruebas) como si fueran los
        datos actuales de la nube.

        Los datos reales permanecen en Supabase y se cargarán únicamente
        después de recuperar/iniciar una sesión válida.
      */
      businesses = [];
      movements = [];
      tasks = [];
      ...
      console.warn(
        '[MULTIGESTIÓN MR] Supabase configurado sin sesión: datos locales de negocios bloqueados.'
      );
    } else {
      businesses =
        await getAll(
  ...
  function renderCloudUI(active) {
    ...
    if ($('cloudUserLabel')) {
      const activeRole = active
        ? c.roleForBusiness(active.id)
        : null;

      const profileName = String(
        c.state.profile?.display_name || ''
      ).trim();
        ...
        online
          ? (
              activeRole === 'principal'
                ? `${profileName || 'CARLOS'} · Colaborador Principal`
                : c.state.user?.is_anonymous
                  ? (profileName || 'Colaborador autorizado')
                  : (
                      profileName
                        ? `${profileName} · Administrador`
                        : (c.state.user?.email || 'Administrador conectado')
                    )
            )
        ...
        (
          ...
          .filter(
            m =>
              m.business_id ===
                activeBusinessId &&
              ['collaborator', 'principal'].includes(m.role)
          );
      ...
      members.forEach(
        member => {
          ...
          row.innerHTML = `
            <div>
              <strong>
                ${member.role === 'principal' ? 'CARLOS · Colaborador Principal' : `Colaborador ${escapeHtml(shortId)}`}
              </strong>
  ...
  function renderAgenda() {
    ...
    const canAddTask = !cloudActive() || !!cloud()?.canAddTask?.(active.id);
    if ($('newTaskBtn')) {
      $('newTaskBtn').disabled = !canAddTask;
      $('newTaskBtn').hidden = !canAddTask;
    }
    ...
    list.forEach((task, index) => {
      ...
      const canEdit = !cloudActive() || !!cloud()?.canEditTask?.(task);
      const canDelete = !cloudActive() || !!cloud()?.canDeleteTask?.(task.businessId);
      tr.innerHTML = `
        <td>${index + 1}</td>
        ...
        <td><div class="row-actions">${canEdit ? '<button class="btn ghost edit-task" type="button">Editar</button>' : ''}${canDelete ? '<button class="btn danger delete-task" type="button">Eliminar</button>' : ''}</div></td>`;
  ...
  (async () => {
    try {
      ...
      */
      ...
      /*
        V1.4.5:
        No precargar negocios/movimientos locales antes de saber si
        Supabase restauró una sesión. Evita que datos antiguos aparezcan
        durante el arranque o queden seleccionados por error.
      */
      businesses = [];
      movements = [];
      tasks = [];
      ...
      */
      ...
      /*
        V1.4.6:
        Si Supabase está configurado pero no pudo restaurarse una sesión,
        no fingimos que "no hay negocios". Mostramos el acceso administrador
        para recuperar la sesión correcta. Al iniciar sesión, el evento
        mg-auth-changed ejecutará refresh() y cargará los negocios reales.
      */
      if (cloud() && !cloudActive() && !cloudAccessDenied() && !cloudAccessPending()) {
        toast('Nube: sesión no iniciada. Inicie sesión para cargar sus negocios.');
        setTimeout(() => {
          try {

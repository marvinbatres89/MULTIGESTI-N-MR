(() => {
  ...
  $('forgotPasswordBtn')?.addEventListener('click',async()=>{const email=normalize($('authEmail').value);if(!email){toast('Escriba primero el correo del administrador');$('authEmail')?.focus();return;}try{await cloud().resetPasswordForEmail(email);toast('Correo de recuperación enviado. Revise su bandeja de entrada.');}catch(err){toast(err.message||'No se pudo enviar el correo de recuperación');}});
  $('closeResetPasswordDialog')?.addEventListener('click',()=>$('resetPasswordDialog')?.close());
  $('resetPasswordForm')?.addEventListener('submit',async e=>{e.preventDefault();const password=$('newPassword')?.value||'',confirmPassword=$('confirmNewPassword')?.value||'';if(password.length<6)return toast('La contraseña debe tener al menos 6 caracteres');if(password!==confirmPassword)return toast('Las contraseñas no coinciden');try{await cloud().updatePassword(password);$('resetPasswordForm')?.reset();$('resetPasswordDialog')?.close();$('authDialog')?.close();toast('Contraseña actualizada. Ya puede iniciar sesión.');}catch(err){toast(err.message||'No se pudo actualizar la contraseña');}});
  window.addEventListener('mg-password-recovery',()=>{try{if(!$('resetPasswordDialog')?.open)$('resetPasswordDialog')?.showModal();}catch(_){}});

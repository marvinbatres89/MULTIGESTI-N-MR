  ...
  */
  ...
  async function init() {
    ...
    */
    ...
    /*
      V1.4.6 - RESTAURACIÓN ROBUSTA DE SESIÓN
      ---------------------------------------
      getSession() lee primero la sesión persistida. Si existe una sesión
      pero el token necesita renovarse, intentamos refreshSession().
      Esto evita iniciar la interfaz como "sin sesión" mientras Supabase
      todavía está restaurando/renovando la autenticación guardada.
    */
    let {
      data,
      error
    ...
    if (error) {
      console.warn('[MULTIGESTIÓN MR] getSession:', error);
    }
    ...
    if (restoredSession) {
      try {
        ...
        if (!refreshed.error && refreshed.data?.session) {
          restoredSession = refreshed.data.session;
        }
      } catch (refreshError) {
        console.warn('[MULTIGESTIÓN MR] refreshSession:', refreshError);
      }
    }

    state.session = restoredSession;
    state.user = restoredSession?.user || null;
    state.initialized = true;

    console.info(
      '[MULTIGESTIÓN MR] Sesión restaurada:',
      state.user ? state.user.id : 'NINGUNA'
    );

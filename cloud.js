(() => {
  ...
  /*
     V1.4.2 - SESIÓN PERSISTENTE
     --------------------------------
     El enlace privado continúa siendo de un solo uso.
     Después de canjearlo correctamente, la sesión queda
     guardada en el dispositivo para que el colaborador
     principal pueda volver a entrar desde la URL normal
     sin necesitar un enlace nuevo.

     Compatibilidad:
     si una sesión válida de V1.4.1 todavía existe en
     sessionStorage, se migra automáticamente a localStorage
     la primera vez que Supabase solicita esa clave.
  */
  ...
  function canAddTask(businessId) {
    return ['admin', 'collaborator', 'principal'].includes(roleForBusiness(businessId));
  }

  function canEditTask(task) {
    return ['admin', 'collaborator', 'principal'].includes(roleForBusiness(task.businessId));
  }

  function canDeleteTask(businessId) {
    return ['admin', 'principal'].includes(roleForBusiness(businessId));
  }
  ...
  async function fetchData() {
    ...
    /*
      V1.4.4 - SINCRONIZACIÓN RESISTENTE

      NEGOCIOS es la consulta principal. Si Supabase puede devolver
      los negocios, un fallo independiente en movimientos, agenda,
      miembros o perfil NO debe hacer desaparecer "Mis negocios".
    */
    ...
    if (bRes.error) {
      throw new Error(
        `No se pudieron cargar los negocios: ${bRes.error.message || bRes.error}`
      );
    }
    ...
    const warnings = [];
    const [mSettled, tSettled, memberSettled] = await Promise.allSettled([
      client
        .from('movements')
        .select('*')
    ...
    const settledResponse = (settled, label) => {
      if (settled.status === 'rejected') {
        warnings.push(`${label}: ${settled.reason?.message || settled.reason || 'error de conexión'}`);
        return null;
      }
      if (settled.value?.error) {
        warnings.push(`${label}: ${settled.value.error.message || settled.value.error}`);
        return null;
      }
      return settled.value;
    };

    const mRes = settledResponse(mSettled, 'Movimientos');
    const tRes = settledResponse(tSettled, 'Agenda');
    const memberRes = settledResponse(memberSettled, 'Miembros');

    state.movements = mRes ? (mRes.data || []).map(mapMovement) : [];
    state.tasks = tRes ? (tRes.data || []).map(mapTask) : [];
    state.members = memberRes ? (memberRes.data || []) : [];

    /* Perfil visible del usuario actual.
       Si no puede leerse, la sincronización principal continúa. */
    state.profile = null;
    try {
      ...
      if (!profileRes.error) {
        ...
        warnings.push(`Perfil: ${profileRes.error.message || profileRes.error}`);
      }
    } catch (err) {
      state.profile = null;
      warnings.push(`Perfil: ${err?.message || err || 'error de conexión'}`);
    }

    if (warnings.length) {
      console.warn('[MULTIGESTIÓN MR] Sincronización parcial:', warnings);
    }
  ...
  /* =========================================================
     DIAGNÓSTICO SEGURO DE NUBE - V1.4.5
     No expone contraseña, access token, refresh token ni claves.
     ========================================================= */
  async function getCloudDiagnostic() {
    const user = state.user;
    if (!user) {
      return {
        sessionActive: false,
        userIdShort: 'sin sesión',
        emailMasked: '—',
        businesses: [],
        warnings: ['No hay una sesión autenticada activa.']
      };
    }
    ...
    const maskEmail = value => {
      ...
      if (!domain) return 'no disponible';
    ...
    if (bRes.error) {
      throw new Error(`Diagnóstico de negocios: ${bRes.error.message || bRes.error}`);
    }
    ...
    try {
      const mRes = await client
        ...
        .select('business_id,user_id,role,active')
        .eq('user_id', user.id);
      if (mRes.error) warnings.push(`Miembros: ${mRes.error.message || mRes.error}`);
      else memberships = mRes.data || [];
    } catch (err) {
      warnings.push(`Miembros: ${err?.message || err || 'error'}`);
    }

    const memberByBusiness = new Map(
      memberships.map(m => [String(m.business_id), m])
    );

    const businesses = (bRes.data || []).map(b => {
      const owned = String(b.owner_id || '') === String(user.id);
      const membership = memberByBusiness.get(String(b.id));
      let access = owned ? 'PROPIETARIO' : 'AUTORIZADO POR SUPABASE';
      if (!owned && membership) {
        access = `COLABORADOR · ${String(membership.role || 'miembro').toUpperCase()}${membership.active === false ? ' · INACTIVO' : ''}`;
      }
      return {
  ...
  async function deleteTask(id) {
    ...
    if (!canDeleteTask(task.businessId)) {
      throw new Error('No tiene permiso para eliminar tareas.');
    }

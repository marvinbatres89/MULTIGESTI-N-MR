(() => {
  ...
  async function init() {
    ...
    // Registrar el observador ANTES de leer la sesión. En los enlaces de
    // recuperación Supabase puede emitir PASSWORD_RECOVERY durante el arranque.
    client.auth.onAuthStateChange((event, session) => {
      state.session = session || null;
      state.user = session?.user || null;
      if (event === 'PASSWORD_RECOVERY') {
        state.recoveryMode = true;
        window.dispatchEvent(new CustomEvent('mg-password-recovery'));
      }
    ...
    // Respaldo para navegadores donde el evento PASSWORD_RECOVERY se produjo
    // antes de que la interfaz terminara de cargar.
    if (recoveryInUrl && state.session) {
      state.recoveryMode = true;
      setTimeout(() => window.dispatchEvent(new CustomEvent('mg-password-recovery')), 0);
    }
  ...
  async function resetPasswordForEmail(email) {
    const redirectTo = 'https://marvinbatres89.github.io/MULTIGESTI-N-MR/';
